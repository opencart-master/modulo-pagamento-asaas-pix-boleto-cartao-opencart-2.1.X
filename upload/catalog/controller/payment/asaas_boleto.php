...
class ControllerPaymentAsaasBoleto extends Controller {
	...
	public function confirm() {
		...
		if (isset($this->session->data['payment_method']['code']) && $this->session->data['payment_method']['code'] == 'asaas_boleto') {
			...
			"dueDate" => date('Y-m-d', strtotime('+' . $this->config->get('asaas_boleto_venc') .' days')),
		    ...
		    $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('asaas_boleto_order_status_id'), $comment, true);
