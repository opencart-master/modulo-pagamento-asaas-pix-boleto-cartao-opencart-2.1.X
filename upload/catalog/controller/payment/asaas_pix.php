...
class ControllerPaymentAsaasPix extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/asaas_pix.tpl')) {
			...
			return $this->load->view('default/template/payment/asaas_pix.tpl', $data);
		}
	}
	...
	public function confirm() {
		$this->load->language('payment/asaas_pix');
		$json = array();
		if (isset($this->session->data['payment_method']['code']) && $this->session->data['payment_method']['code'] == 'asaas_pix') {
			...
			foreach ($custom as $key => $value) {
				if ($this->config->get('asaas_pix_doc') == $key && !empty($value)) {
                   $doc = $value;
				} 
				
				if ($this->config->get('asaas_pix_doc1') == $key && !empty($value)) {
				   $doc = $value;
				}
			}
