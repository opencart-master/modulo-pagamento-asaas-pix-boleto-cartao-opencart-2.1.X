<?php
class ControllerPaymentAsaasPix extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('payment/asaas_pix');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('asaas_pix', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/asaas_pix', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		if (isset($this->request->post['asaas_pix_api_key'])) {
			$data['asaas_pix_api_key'] = $this->request->post['asaas_pix_api_key'];
		} else {
			$data['asaas_pix_api_key'] = $this->config->get('asaas_pix_api_key');
		}

		if (!empty($this->config->get('asaas_pix_api_key'))) {
            $data['show'] = true;
		} else {
		    $data['show'] = false;
		}

		if (isset($this->request->post['asaas_pix_order_status_id'])) {
			$data['asaas_pix_order_status_id'] = $this->request->post['asaas_pix_order_status_id'];
		} else {
			$data['asaas_pix_order_status_id'] = $this->config->get('asaas_pix_order_status_id');
		}

		if (isset($this->request->post['asaas_pix_order_status_id2'])) {
			$data['asaas_pix_order_status_id2'] = $this->request->post['asaas_pix_order_status_id2'];
		} else {
			$data['asaas_pix_order_status_id2'] = $this->config->get('asaas_pix_order_status_id2');
		}

		if (isset($this->request->post['asaas_pix_order_status_id3'])) {
			$data['asaas_pix_order_status_id3'] = $this->request->post['asaas_pix_order_status_id3'];
		} else {
			$data['asaas_pix_order_status_id3'] = $this->config->get('asaas_pix_order_status_id3');
		}

		if (isset($this->request->post['asaas_pix_order_status_id4'])) {
			$data['asaas_pix_order_status_id4'] = $this->request->post['asaas_pix_order_status_id4'];
		} else {
			$data['asaas_pix_order_status_id4'] = $this->config->get('asaas_pix_order_status_id4');
		}

		if (isset($this->request->post['asaas_pix_order_status_id5'])) {
			$data['asaas_pix_order_status_id5'] = $this->request->post['asaas_pix_order_status_id5'];
		} else {
		...
		if (isset($this->request->post['asaas_pix_status'])) {
			$data['asaas_pix_status'] = $this->request->post['asaas_pix_status'];
		} else {
			$data['asaas_pix_status'] = $this->config->get('asaas_pix_status');
		}
		...
		if (isset($this->request->post['asaas_pix_sort_order'])) {
			$data['asaas_pix_sort_order'] = $this->request->post['asaas_pix_sort_order'];
		} else {
			$data['asaas_pix_sort_order'] = $this->config->get('asaas_pix_sort_order');
		}

		if (isset($this->request->post['asaas_pix_doc'])) {
			$data['asaas_pix_doc'] = $this->request->post['asaas_pix_doc'];
		} else {
			$data['asaas_pix_doc'] = $this->config->get('asaas_pix_doc');
		}

		if (isset($this->request->post['asaas_pix_doc1'])) {
			$data['asaas_pix_doc1'] = $this->request->post['asaas_pix_doc1'];
		} else {
		...
		$this->response->setOutput($this->load->view('payment/asaas_pix.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'payment/asaas_pix')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		if (empty($this->request->post['asaas_pix_api_key'])) {
			$this->error['key'] = $this->language->get('error_key');
		}

		if (!isset($this->request->post['asaas_pix_doc']) || $this->request->post['asaas_pix_doc'] == 0 ) {
